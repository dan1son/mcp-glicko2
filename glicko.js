...
(function(exports){
	...
	player.rating2 = (player.rating - DEFAULT_RATING)/173.7178
...
*/
	...
	var	makef = function(delta, v, a){
		  return function(x){
		    return Math.exp(x) * (Math.pow(delta, 2) - Math.pow(p.rd2, 2) - v - Math.exp(x)) / (2*Math.pow(Math.pow(p.rd2, 2) + v + Math.exp(x), 2)) - (x - a) / Math.pow(t, 2);
		  };
		};

			
	// Step 5 (ripped and modded from glicko2js, this one was broken somehow
		var a = Math.log(Math.pow(p.vol, 2));
		var f = makef(delta, v, a);
		var epsilon = 0.000001;
		var A = a;
		var B, k;

		if (Math.pow(delta, 2) > Math.pow(p.rd2, 2) + v) {
			B = Math.log(Math.pow(delta, 2) - Math.pow(p.rd2, 2) - v);
		} else {
			k = 1;
			while (f(a - k * t) < 0) {
				k = k + 1;
...
//
//	//5.4
//	if (player.name == 'warwickftw') 
//    	console.log("uhhh: " + (B - A));
...
//	var vol_prime = Math.exp(A/2);
	...
	if (opponents.length != 0 ) {
		...
		p.rd2 = Math.sqrt(Math.pow(p.rd2,2) + Math.pow(p.vol,2));;
	}

	//step 8 convert back to original scale
	p.rating = p.rating2 * 173.7178 + DEFAULT_RATING;
	p.rd = p.rd2 * 173.7171;
